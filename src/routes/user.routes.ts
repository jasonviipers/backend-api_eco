...
import { addressSchema, changePasswordSchema, updateProfileSchema } from '../schemas/user'
...
userRouter.put(
  ...
  asyncHandler(async (c) => {
    ...
    const userInfo = await query('SELECT email, full_name FROM users WHERE id = $1', [userId])
    // await sendEmail({
...
userRouter.post(
  ...
  asyncHandler(async (c) => {
    ...
    if (addressData.isDefault) {
      await query('UPDATE addresses SET is_default = false WHERE user_id = $1 AND type = $2', [
        userId,
        addressData.type,
      ])
    }
...
userRouter.put(
  ...
  asyncHandler(async (c) => {
    ...
    const existingAddress = await query('SELECT id, type FROM addresses WHERE id = $1 AND user_id = $2', [
      id,
      userId,
    ])
...
userRouter.delete(
  ...
  asyncHandler(async (c) => {
    ...
    // Check if address belongs to user
    const existingAddress = await query('SELECT id FROM addresses WHERE id = $1 AND user_id = $2', [id, userId])
...
userRouter.get(
  ...
  `,
    ...
    const countResult = await query('SELECT COUNT(*) as total FROM wishlists WHERE user_id = $1', [userId])
...
userRouter.post(
  ...
  asyncHandler(async (c) => {
    ...
    const existingWishlist = await query('SELECT id FROM wishlists WHERE user_id = $1 AND product_id = $2', [
      userId,
      productId,
    ])
...
userRouter.delete(
  ...
  asyncHandler(async (c) => {
    ...
    const result = await query('DELETE FROM wishlists WHERE user_id = $1 AND product_id = $2', [userId, productId])
...
userRouter.post(
  ...
  asyncHandler(async (c) => {
    ...
    const vendorResult = await query('SELECT id FROM vendors WHERE id = $1', [vendorId])
    ...
    const existingFollow = await query('SELECT id FROM vendor_followers WHERE user_id = $1 AND vendor_id = $2', [
      userId,
      vendorId,
    ])
    ...
    if (existingFollow.rows.length > 0) {
      // Unfollow
      await query('DELETE FROM vendor_followers WHERE user_id = $1 AND vendor_id = $2', [userId, vendorId])
      await query('UPDATE vendors SET follower_count = follower_count - 1 WHERE id = $1', [vendorId])

      return c.json({ message: 'Vendor unfollowed', following: false })
    } else {
      // Follow
      await query('INSERT INTO vendor_followers (user_id, vendor_id) VALUES ($1, $2)', [userId, vendorId])
      await query('UPDATE vendors SET follower_count = follower_count + 1 WHERE id = $1', [vendorId])
...
userRouter.get(
  ...
  `,
    ...
    const countResult = await query('SELECT COUNT(*) as total FROM notifications WHERE user_id = $1', [userId])

    const notifications = notificationsResult.rows
    const total = Number.parseInt(countResult.rows[0].total)
    const unreadCount = notifications.filter((notification: { is_read: boolean }) => !notification.is_read).length
