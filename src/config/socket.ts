...
export const setupSocketIO = (io: SocketIOServer): void => {
  ...
  io.on("connection", (socket: AuthenticatedSocket) => {
    ...
    socket.on("join_stream", async (streamId: string) => {
      ...
      const roomSize = io.sockets.adapter.rooms.get(`stream_${streamId}`)?.size || 0
    ...
    // Chat events
    socket.on("send_message", async (data: { streamId: string; message: string }) => {
      const messageData = {
      ...
      await executeQuery(
        ...
        [data.streamId, messageData.id, socket.userId, data.message, new Date()],
      )
...
export const emitToStream = (io: SocketIOServer, streamId: string, event: string, data: any): void => {
  io.to(`stream_${streamId}`).emit(event, data)
}

export const emitToOrder = (io: SocketIOServer, orderId: string, event: string, data: any): void => {
