...
class VideoProcessingQueue {
  ...
  async addJob(videoId: string, videoUrl: string, options: ProcessingOptions = {}): Promise<void> {
    const job: VideoJob = {
  ...
  private async processQueue(): Promise<void> {
    ...
    logger.info(`Starting video processing queue with ${this.queue.length} jobs`)
  ...
  private async processJob(job: VideoJob): Promise<void> {
    try {
      ...
      await query("UPDATE short_videos SET processing_status = $1 WHERE id = $2", ["processing", job.videoId])
      ...
      if (job.retryCount < job.maxRetries) {
        // Retry the job
        logger.info(`Retrying job: ${job.id} (attempt ${job.retryCount + 1}/${job.maxRetries})`)
        setTimeout(() => {
        ...
        // Mark as failed
        await query("UPDATE short_videos SET processing_status = $1 WHERE id = $2", ["failed", job.videoId])
