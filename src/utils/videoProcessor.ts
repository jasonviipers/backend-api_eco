...
export const processVideo = async (
	...
	try {
		...
		const formats = await processVideoFormats(
			inputPath,
			tempDir,
			metadata,
			options,
...
const processVideoFormats = async (
	...
	const availableFormats = DEFAULT_FORMATS.filter((format) => {
		...
		return (
			height <= metadata.height && requestedFormats.includes(format.quality)
		);
	});
	...
	if (
		!availableFormats.find(
			(f) =>
				f.quality === getQualityFromResolution(metadata.width, metadata.height),
		)
	) {
	...
	for (const format of availableFormats) {
		try {
			...
			logger.info(
				`Processed format ${format.quality}: ${uploadResult.secure_url}`,
			);
		} catch (error) {
...
const generateThumbnails = async (
	...
	for (let i = 0; i < timestamps.length; i++) {
		try {
			...
			logger.info(
				`Generated thumbnail ${i + 1}/${count}: ${uploadResult.secure_url}`,
			);
		} catch (error) {
...
const addWatermark = (command: any, watermark: any): any => {
	if (watermark.text) {
		...
		const position = getWatermarkPosition(watermark.position || "bottom-right");
		command = command.outputOptions([
			`-vf overlay=${position.x}:${position.y}`,
		]);
	}
...
export const generateVideoGif = async (
	videoUrl: string,
	startTime = 0,
	duration = 3,
): Promise<string> => {
	...
	try {
		...
		await new Promise<void>((resolve, reject) => {
			ffmpeg(inputPath)
				...
				.fps(10)
				.outputOptions([
					"-vf scale=320:180:flags=lanczos,palettegen=reserve_transparent=0",
					"-f gif",
				])
				.output(outputPath)
				.on("end", () => resolve())
...
export const batchProcessVideos = async (
	...
	for (let i = 0; i < videoUrls.length; i += concurrency) {
		...
		const batchPromises = batch.map(async (url, index) => {
			try {
				logger.info(
					`Processing video ${i + index + 1}/${videoUrls.length}: ${url}`,
				);
		...
		batchResults.forEach((result, index) => {
			if (result.status === "fulfilled") {
				...
				logger.error(
					`Video ${i + index + 1} processing failed:`,
					result.reason,
				);
			}
		});
